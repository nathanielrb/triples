      ...
      (if (null? triples)
          (*DB*
           (make-db
            ...
            (map-add (db-indexes (*DB*)) time (make-index si spi pi poi oi osi spoi))))
          (match (car triples)
            ((s p o)
             (loop (cdr triples)
                   (map-update-in si `(,s) consr p)
                   (map-update-in spi `(,(index-key s p)) consr o)
                   (map-update-in pi `(,p) consr  o)
                   (map-update-in poi `(,(index-key p o)) consr s)
                   (map-update-in oi `(,o) consr s)
                   (map-update-in osi `(,(index-key o s)) consr p)
                   (map-add spoi (index-key s p o) val))))))))
